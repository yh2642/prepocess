...
def dist_part_table(dict):
	for key in dict:
		for zone in dict[key]:
			...
			cur=conn.cursor()
			cur.execute("""SET NAMES utf8""")
			cur.execute("""Insert into %s select * from temp_info2 where memberid >= '%s' and memberid < '%s';""" % ('offer_info' + key, zone[0], zone[1]))
			t2 = time.clock()
	...
	'''
	for indx in range(1, len(char_ls)):
		conn=MySQLdb.connect(host='localhost',user='root',passwd='root',db='test2',port=3306, charset='utf8')
		cur=conn.cursor()
		cur.execute("""SET NAMES utf8""")
		cur.execute("""select memberid, category, qualitylevel from offer_info where memberid >= %s and memberid < %s""", (char_ls[indx-1], char_ls[indx]))
...
if __name__ == '__main__':
	...
	t1 = time.clock()
	test(table_dict, num_of_threads=8)
